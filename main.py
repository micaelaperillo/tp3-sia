...
if __name__ == '__main__':
    ...
    for learning_rate in learning_rates:
        for epoch_amount in epochs:
            ...
            and_breaking_epoch, and_training_error = and_simple_perceptron.linear_training(and_data, and_labels)
            first_exercise_results_file.write(f"{seed},and,{learning_rate},{and_breaking_epoch},{and_training_error}\n")

            xor_simple_perceptron = Perceptron(len(xor_data[0]), learning_rate, epoch_amount)            
            xor_breaking_epoch, xor_training_error = xor_simple_perceptron.linear_training(xor_data, xor_labels)
            first_exercise_results_file.write(f"{seed},xor,{learning_rate},{xor_breaking_epoch},{xor_training_error}\n")
