...
class Perceptron:
    ...
    def train(self, training_set:List[List[int]], labels:List[int], testing_set:List[List[int]], test_labels:List[int], learning_rate:float, epochs:int, optimizer, error_function:ErrorFunctionType, max_acceptable_error:float,file, method:str, is_activation_derivable=False, beta:float= 1.0, partition:int=0,descale_fun = None):
        for epoch in range(epochs):
            ...
            errors = []
            test_errors = []
            # training error for the current epoch
            ...
            if (testing_set is not None) and (test_labels is not None):
                # testing error for the current epoch
                for x_value, expected_value in zip(testing_set, test_labels):
                    x_value = np.insert(x_value, 0, 1)
                    prediction, h_supra_mu = self.predict(x_value,beta)
                    if descale_fun is not None:
                        prediction = descale_fun(prediction)
                        expected_value = descale_fun(expected_value)
                    test_error = (expected_value - prediction) 
                    test_errors.append(test_error)

                perceptron_testing_error = error_function(np.array(test_errors))
                file.write(f"{self.seed},{partition},{self.weights},{method},{beta},{learning_rate},{epochs},{epoch+1},{perceptron_error},{perceptron_testing_error}\n")
